...
class TSND151:
        ...
        while len(res) < num and not self.is_closed():
            ...
            if len(_b) == 0:
                ...
                if self.is_recording():
                    if (now - last_read_time).total_seconds() > self._serial_property['timeout']:
                        raise TimeoutError()
                elif (now - last_read_time).total_seconds() > ping_check_interval:
                    self._ping()  # send only, do not wait for its own response
                    self._ping_pending += 1
                    if self._ping_pending > self._response_wait_auto_recovery_limit:
                        raise TimeoutError("No response to heartbeat ping")
                    last_read_time = now  # schedule next ping after the interval
            else:
                last_read_time = datetime.datetime.now()
                self._ping_pending = 0  # any received byte means the connection is alive
                res += _b
    ...
    def _in_loop_read_response(self):
        ...
        if error is not None and not self.is_closed():
            if self.auto_recovery:
                # Throttle log to at most once per second (deterministic).
                now = datetime.datetime.now()
                if (self._last_recovery_log_at is None
                        or (now - self._last_recovery_log_at).total_seconds() >= 1.0):
    ...
    def recover_connection(self, error):
        if self.is_closed():
            ...
            self._ping_pending = 0  # reset heartbeat counter for the fresh connection
    ...
    def wait_response(self, code_name, timeout_sec=None):
        ...
        while not self._read_response_thread.check_should_be_stop(raise_error=False):
            try:
        ...
        # The read thread has stopped (connection closed/lost): the queue will
        # never be filled, so report it as documented instead of returning None.
        raise IOError(f"Connection for TSND151 is lost while waiting response for {code_name}")
    ...
    def set_battery_voltage_measurement(self, send=False, save=False):
        ...
        if not isinstance(send, bool):
            raise ValueError(f"Invalid 'send' (have to be bool):{send}")
        if not isinstance(save, bool):
            raise ValueError(f"Invalid 'save' (have to be bool):{save}")
    ...
    def set_option_button_behavior(self, mode: OptionButtonMode):
        ...
        try:
            ...
            raise ValueError(f"Invalid 'mode' (have to be one of the TSND151.OptionButtonMode): {mode}") from e
    ...
    def set_overwrite_protection(self, enable=False):
        ...
        if not isinstance(enable, bool):
            raise ValueError(f"Invalid 'enable', {enable}, (have to be bool).")
    ...
    def _ping(self):
        # Send only. The response is consumed by the normal read loop.
        # Do NOT wait for its own response here: this runs on the read thread
        # itself, so waiting would deadlock (the thread that fills the queue
        # would be blocked waiting on that same queue).
    ...
    def _get_mode(self, timeout_sec=None):
        ...
        """
        # Drop stale heartbeat-ping replies so we wait for the fresh one.
        q = self._response_queue_map[self._RESPONSE_CODE_MAP_['mode']]
        if q is not None:
            while not q.empty():
                try:
                    q.get_nowait()
